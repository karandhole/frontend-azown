...
const PropertyPreview = () => {
    ...
    const getTemplateData = async ()=>{
      ...
      if(res.ok){
        ...
        const myData = json.property_template_data;
        if(myData){
    ...
    // const getPremiumTemplate = async () => {
    //    const res = await fetch(`${host}/api/property/get-primium-template/:${cmrdata._id}`,{
    //     method:"GET",
    //     headers:{
    //       "content-type":'application/json',
    //       "auth-token":JSON.parse(localStorage.getItem("userDetail")).authtoken,
 ...
 setView(!view)
    ...
    const updateTemplateData = async ()=>{
      ...
      const res = await fetch(`${host}/property/update-primium-template/${id}`,{
        method:"PUT",
        headers:{
          "Content-Type": "application/json",
          "auth-token": JSON.parse(localStorage.getItem("userDetail")).authtoken,
