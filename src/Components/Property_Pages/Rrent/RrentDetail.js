...
const RrentDetail = () => {
  ...
  const { rrdata,rrDetail,showLoader} = context;
  const data = rrdata
  ...
  const setting = {
    ...
    slidesToShow: 1,
    slidesToScroll: 1,
    nextArrow: <button title="Next (Right arrow key)" type="button" className="mfp-arrow mfp-arrow-right mfp-prevent-close"></button>,
    prevArrow:<button title="Previous (Left arrow key)" type="button" className="mfp-arrow mfp-arrow-left mfp-prevent-close"></button>,
    autoplay: true,
    autoplaySpeed: 3000,
  ...
  useEffect(() => {
    if(data.rr_location_latitude && data.rr_location_longitude){
      const lat = parseFloat(data.rr_location_latitude);
      const lng = parseFloat(data.rr_location_longitude)
       setMarker({lat:lat,lng:lng})
      
    }
  
  }, [data.rr_location_latitude, data.rr_location_longitude]);
          ...
          <div className="property_block_wrap">
            ...
            <div className="block-body">
              <ul className="row p-0 m-0">
                ...
                <li className="col-lg-6 col-md-6 mb-2 p-0"><img src="assets/icons/kitchen.png" className="icon-details mr-1" />Kitchen Type</li><p className='col-6'>{data.rr_detail_kitchen_type}</p>
          ...
          <div className="property-sidebar">
            <div className="sider_blocks_wrap">
              <div className="side-booking-header">
                ...
                {data && data.primium_expiry_date > todayDate ? <Link to={`/premium-property/${data._id}/${1}`}> <a className="theme-bg rounded text-white p-2 cursor-pointer">View Premium</a></Link>: null }
              </div>
...
{localStorage.getItem('userDetail') && (JSON.parse(localStorage.getItem('userDetail')).user.usertype === '2') ?  (localStorage.getItem("userDetail") ?
                     ...
                     (
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          brokerRequest(data._id,1,data.userid);
                          setcontacted(true);
                           setShowUser(true)
            ...
            <div className="sider_blocks_wrap">
              <div className="side-booking-body">
               ...
               {/* {localStorage.getItem("userDetail")  <Link to={`/user-detail/${data.userid}`} className="agent-btn-contact" ><i className="ti-comment-alt" />View Owner Detail</Link> : <button className='agent-btn-contact w-100 ' style={{cursor:"pointer"}} onClick={()=>setShow(true)}> View Owner Detail</button>} */}
