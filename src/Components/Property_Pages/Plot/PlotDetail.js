...
const PlotDetail = () => {
  ...
  const {  plotDetail,plotdata,showLoader} = context;
  const data = plotdata
  ...
  useEffect(() => {
    if(data.ps_location_latitiude && data.ps_location_longitude){
      const lat = parseFloat(data.ps_location_latitiude);
      const lng = parseFloat(data.ps_location_longitude)
       setMarker({lat:lat,lng:lng})
      
    }
  }, [data.ps_location_latitiude, data.ps_location_longitude]);
  console.log(data)
 const handleClose = ()=>{
  setShowUser(false);
}
          ...
          <div className="property-sidebar">
            <div className="sider_blocks_wrap">
              <div className="side-booking-header">
                ...
                {data && data.primium_expiry_date > todayDate ? <Link to={`/premium-property/${data._id}/${7}`}> <a className="theme-bg rounded text-white p-2 cursor-pointer">View Premium</a></Link> :null}
              </div>
                     ...
                     (
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          brokerRequest(data._id,7,data.userid);
                          setcontacted(true);
                           setShowUser(true)
