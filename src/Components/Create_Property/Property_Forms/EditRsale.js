      ...
      <section className='gray'>
        <div className="container">
          <div className="row justify-content-md-center bg-white">
                ...
                <Formik
                  ...
                  onSubmit={(values, { setSubmitting, setFieldValue,setTouched }) => {
                    if (currentStep===1){
                      ...
                      axios.put(`${host}/property/update-rrs/${rrsdata._id}/1`, formData, {
                        headers: {
           ...
           {rrsdata && <IsPremium propId={rrsdata._id} ownerId={rrsdata.userid} expDate={rrsdata.primium_expiry_date} propType={2} />}
            </div>
          </div>
        </div>
        {rrsdata && <PropertyTemplates id={rrsdata._id} />}
